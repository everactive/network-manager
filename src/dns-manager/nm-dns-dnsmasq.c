...
start_dnsmasq (NMDnsDnsmasq *self)
{
	...
	/* Start dnsmasq */
	if ((app_data_path = get_snap_app_data_path())) {
		pid_file = g_strdup_printf ("%s%s", app_data_path, PIDFILE);
		nm_log_warn (LOGD_DNS, "start_dnsmasq: pid_file: %s", pid_file);

		conf_file = g_strdup_printf ("%s%s", app_data_path, CONFFILE);
		nm_log_warn (LOGD_DNS, "start_dnsmasq: conf_file: %s", conf_file);

		dir_name = g_strdup_printf ("%s%s", app_data_path, NMRUNDIR);
		nm_log_warn (LOGD_DNS, "start_dnsmasq: dir_name: %s", dir_name);

		if (!g_file_test (dir_name, G_FILE_TEST_EXISTS | G_FILE_TEST_IS_DIR)) {
			if (!g_mkdir_with_parents (dir_name, 0755)) {
				nm_log_warn (LOGD_DNS, "start_dnsmasq: failed to create dnsmasq config dir: %s", dir_name);
			}
		}
	} else {
		pid_file = g_strdup_printf (PIDFILE);
		conf_file = g_strdup_printf (CONFFILE);
	}
	...
	if (!g_file_set_contents (conf_file, conf->str, -1, &error)) {
	...
	if (get_snap_app_path ())
		app_path = g_strdup_printf ("%s/usr/sbin/dnsmasq", get_snap_app_path ());
	else
		app_path = g_strdup (find_dnsmasq ());

	nm_log_warn (LOGD_DNS, "dnsmasq app_path: %s", app_path);
	...
	pid = nm_dns_plugin_child_spawn (NM_DNS_PLUGIN (self), argv, pid_file, "bin/dnsmasq");
